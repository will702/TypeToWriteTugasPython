...
class Fung:
    ...
    def prosesText(self):
        #proses analisa teks (otamatis enter jika berlebih, dan memecah ke list)
    ...
    def prosesNulis(self): #proses menempelkan teks ke kertas
        ...
        if platform == 'android':
            self.lokasi = f"/sdcard/DCIM/TypeToWrite/{randoming_string()}.png"
            if os.path.exists(self.lokasi) == True:
            ...
            if os.path.exists(f'/sdcard/DCIM/TypeToWrite/') == True:
                ...
                self.kertas.save(self.lokasi)
        if platform != 'android':
            self.lokasi = f"{randoming_string()}.png"
            if os.path.exists(self.lokasi) == True:
